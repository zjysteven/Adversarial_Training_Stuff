...
import arguments
import utils
from attacks import Linf_PGD, Linf_PGD_for_CAT_my
...
class CAT():
    def __init__(self, model, optimizer, scheduler, writer, save_path=None, **kwargs):
        ...
        self.criterion = utils.CE_with_soft_label()
        self.num_classes = 10
    ...
    def train(self, epoch):
        ...
        for inputs, targets, idx in batch_iter:
            ...
            if not self.fixed_alpha:
                self.attack_cfg['alpha'] = 2*eps_per_sample/self.attack_cfg['steps']

            # generate adversarial examples
            adv_inputs = Linf_PGD_for_CAT_my(self.model, inputs, soft_targets, eps=eps_per_sample, **self.attack_cfg)
            ...
            outputs = self.model(adv_inputs)
            # use soft label
            loss = self.criterion(outputs, soft_targets)
            losses += loss.item()
...
def main():
    ...
    #filename = 'model'
    save_root = os.path.join('checkpoints', 'cat_my', '{:s}{:d}-{:d}_seed_{:d}'.format(args.arch, args.depth, args.width, args.seed))
    if args.fixed_alpha:
        save_root += '_fixed_alpha'
