...
import arguments
import utils
from attacks import Linf_PGD_for_CAT_minhao_apex, Linf_PGD
...
class CAT():
    def __init__(self, model, optimizer, scheduler, writer, save_path=None, **kwargs):
        ...
        self.attack_cfg = {'alpha': kwargs['alpha'],
                           'steps': kwargs['steps'],
                           'is_targeted': False,
                           'rand_start': True, # see the pseudo-code of CAT
    ...
    def train(self, epoch):
        ...
        for inputs, targets, idx in batch_iter:
            ...
            adv_inputs = Linf_PGD_for_CAT_minhao_apex(self.model, self.optimizer, inputs, soft_targets if self.label_smoothing else targets, eps=eps_per_sample, **self.attack_cfg)
...
def get_args():
    ...
    arguments.apex_args(parser)
    arguments.apex_ddp_args(parser)
...
def main():
    ...
    #filename = 'model'
    save_root = os.path.join('checkpoints', 'cat_minhao_apex', '{:s}{:d}-{:d}_seed_{:d}_epochs_{:d}_{:s}'.format(args.arch, args.depth, args.width, args.seed, args.epochs, args.lr_sch))
    if args.lr_sch == 'cyclic':
        save_root += '_{:.1f}'.format(args.lr_max)
    if args.fixed_alpha:
        save_root += '_fixed_alpha_{:.5f}'.format(args.alpha)
    else:
        save_root += '_adapt_alpha_{:.2f}xeps'.format(args.adapt_alpha)
    save_root += '_apex_{:s}'.format(args.opt_level)
    ...
    # initialize models and get optimizers and schedulers
    model, optimizer, scheduler = utils.get_model_opt_sch_apex_ddp(args)
